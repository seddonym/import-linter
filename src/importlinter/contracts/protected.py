...
class AllowListContract(Contract):
    """
    Allow-list contracts check that one set of modules is only directly imported by another set of modules.
    Configuration options:
        - protected_modules:    The modules that must not be imported except by the list of importers, and by each other.
        ...
        - ignore_imports:       A set of ImportExpressions. These imports will be ignored if the import
                                 ...
                                 the contract be kept instead. (Optional.) Anything in this list is tech debt.
    ...
    def check(self, graph: grimp.ImportGraph, verbose: bool) -> ContractCheck:
        ...
        protected_modules = self._resolve_module_expressions(self.protected_modules, graph)
        allowed_modules = self._resolve_module_expressions(self.allowed_importers, graph)
